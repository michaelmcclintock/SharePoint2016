	...
	[
		...
		secretKeyVariable: 'AWS_SECRET_ACCESS_KEY'
            ...
            //withCredentials([string(credentialsId: 'c8178bdb-a72f-4ede-91d3-190d6a11af15', variable: 'AWS_ACCESS_KEY_ID'), string(credentialsId: 'db568c29-c2e0-407b-a93b-463f604a3553', variable: 'AWS_SECRET_ACCESS_KEY')]) {
                ...
                //Remove the terraform state file so we always start from a clean state
                sh "terraform remote config -backend=s3 -backend-config='bucket=us-west-2a.terraform.remotestate.sb' -backend-config='key=terraform.tfstate' -backend-config='region=us-west-2' -backend-config='encrypt=true'"
